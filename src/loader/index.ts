...
import { type Loader, UnitLoadFailedError, type Units, type UnitUris } from "./types";
import { loadUnitConfig, loadUnitRecord, validateUnitRecord } from "./util";
	...
	async loadUnits(log: (message: string) => void): Promise<boolean> {
		...
		for (const key of UNIT_KEYS) {
			try {
				...
				const unit = await import(/* @vite-ignore */ record.src).then((mod) => mod.default);
				await testUnit(key, unit, true);
				units[key] = unit;
			} catch (e) {
				success = false;
